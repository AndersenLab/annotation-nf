...
our $REVISION = '$Revision: 638e9157fb26fc0934d4f30caa708be395a0754c $';
our $DATE =	'$Date: 2022-10-20 22:37:42 -0400 (Thu, 20 Oct 2022) $';  
...
while (<EVF>) {
	...
	$field[2] =~ m/^[\w\-\.\@\/\(\)]*?:([\w\.\-]+?):exon\d+:c.([\w\->]+)(:p.([\w\*]+))?/ or die "Error: invalid record found in exonic_variant_function file (exonic format error): <$_>";  #chromsome name can contain dot or - sign, additional change to handle gene name with "()" (20210105) changed the first field from +? to *? to handle Maker output where gene name is empty (20221020)
	
	while ($field[2] =~ m/[\w\-\.\@\/\(\)]+?:([\w\.]+?):exon\d+:c.([\w>]+)(:p.([\w\*]+))?/g) {		#to be consistent with the format above (20210105)
...
if ($newevf) {
	...
	while (<EVF>) {
		...
		for my $i (0 .. @notation-1) {
			...
			$notation[$i] =~ m/^([\w\-\.\@\/\(\)]+?):([\w\.\-]+?):(exon\d+):(c.[\w\->]+)/ or die "Error: invalid notation found: <$notation[$i]>";		#to handle 'LOC119877625:id-LOC119877625:exon2:c.G60T:p.G20G' in canFam4_refGene
...
=head1 SYNOPSIS
 ...
 Version: $Date: 2022-10-20 22:37:42 -0400 (Thu, 20 Oct 2022) $
